...
public class RedisClient{
    ...
    /**
     * 添加一个短暂key，如果key存在，返回false，如果不存在，返回true
     * @param key
     * @param value
     * @param timeout
     * @param unit
     */
    public Boolean set(String key, String value, final long timeout, final TimeUnit unit, Boolean b) {
        ...
        return this.redisTemplate.execute(new RedisCallback<Boolean>() {
            ...
            public Boolean doInRedis(RedisConnection redisConnection) throws DataAccessException {
                Boolean status = redisConnection.setNX(rawKey, rawValue);
                if(status){
                    redisConnection.pExpire(rawKey, rawTimeout);
                }
                return status;
            }
